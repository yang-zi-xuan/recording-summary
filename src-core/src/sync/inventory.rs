...
pub fn build_inventory(
    ...
    //   只从状态表里补,不凭空造(状态表里没有 = 从没同步过 = 没什么可说)。
...
mod tests {
    ...
    fn builds_project_group_file_tree() {
        ...
        let inv = build_inventory(&files, &sel, &SyncState::new(), &BTreeMap::new()).unwrap();
    ...
    fn file_count_and_scope_count() {
        ...
        let inv2 = build_inventory(&files, &sel, &SyncState::new(), &BTreeMap::new()).unwrap();
    ...
    fn synced_files_show_synced() {
        ...
        let inv = build_inventory(&files, &SyncSelection::all(), &state, &remote).unwrap();
        let n = inv
